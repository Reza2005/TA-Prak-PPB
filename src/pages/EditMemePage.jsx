...
function EditMemePage({ memeToEdit, setCurrentPage, onUpdateSuccess }) {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      // Operasi UPDATE: update title, description, category, tags
      const tagsArray = tags ? tags.split(",").map((t) => t.trim()) : [];
      await updateMeme(memeToEdit.id, {
        title,
        description,
      ...
      setStatusMessage(
        `❌ Gagal memperbarui meme: ${error.message || "Error tidak diketahui"}`
      );
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="p-8 max-w-lg mx-auto cream-theme shadow-xl rounded-lg card-accent">
      ...
      <div className="mb-4 text-center">
        <img
          src={memeToEdit.image_url}
          alt="Meme to Edit"
          className="w-full max-h-48 object-contain rounded-md border"
        />
        <p className="text-xs text-gray-500 mt-2">
          Gambar tidak dapat diganti dari sini.
        </p>
      </div>
      ...
      >
        <GalleryHorizontal size={16} className="mr-2" /> Batalkan & Kembali ke
        Galeri
      </button>
    </div>
  );
